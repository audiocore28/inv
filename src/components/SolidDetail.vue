...
<template>
  <div>
    <h1 class="font-oswald font-semibold text-md capitalize m-2">{{ `${item.brand} ${item.series} ${formatSize(item.capacity)}` }}</h1>
    ...
    <div class="flex bg-slate-600 text-slate-500">
      <span class="text-green-500 inline-flex items-center gap-2 px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200">
      ...
      <span v-if="item.rpm !== ''" class="inline-flex items-center gap-2 px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200">
