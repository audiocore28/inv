...
const { filteredDisks } = storeToRefs(diskStore);
</script>

<template>

  <div class="bg-slate-900 flex flex-wrap gap-4 justify-center text-lg font-serif">
    <div v-for="disk in filteredDisks" href="#" class=" relative bg-slate-700 flex-grow text-slate-300 border-l-5 border-green-600 rounded-l-md w-full md:w-5/12 lg:w-3/12">
      ...
      <div class="flex bg-slate-500 text-slate-300">
        <span class="inline-flex items-center gap-2 px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200">
          ...
          <span class="font-inter text-xs">{{ disk.form }}"</span>
        </span>

        <span v-if="disk.rpm !== ''" class="inline-flex items-center gap-2 px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200">
          ...
          <span class="font-inter text-xs">{{ disk.rpm }}</span>
        </span>
        
        <span class="inline-flex items-center gap-2 px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200">
          <svg class="w-4 h-4" width="64px" height="64px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="currentColor"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>Health</title> <rect width="24" height="24" fill="none" stroke="#000000" stroke-width="0" opacity="0.01"></rect> <path d="M17.5,13.5h-4v4h-3v-4h-4v-3h4v-4h3v4h4ZM12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Z" transform="translate(0 0)"></path> </g></svg>
          <span class="font-inter text-xs">{{ disk.health }}%</span>
        </span>
        ...
        <span class="inline-flex items-center gap-2 px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200">
          ...
          <span class="font-inter text-xs">{{ disk.year }}</span>
        </span>
